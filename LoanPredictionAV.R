...
rm(list = ls(all=T))
train <- read.csv("C:/Users/Dell/Desktop/train_AV.csv", header = T, sep = ",", na.strings = "")
test <- read.csv("C:/Users/Dell/Desktop/test_AV.csv", header = T, sep = ",", na.strings = "")
####
...
misval_train <- data.frame(sapply(train, function(x) sum(is.na(x)))) # checking missing values column wise
...
test$LoanAmount[is.na(test$LoanAmount)] <- getmode(test$LoanAmount)
...
train_num$ApplicantIncome <- NULL
train_num$CoapplicantIncome <- NULL
...
####
final_train <- cbind(train_num,train_cat,Loan_Status)
...
test_num$ApplicantIncome <- NULL
test_num$CoapplicantIncome <- NULL
...
model <- train(Loan_Status ~ ., data=final_train, trControl=train_control, method="rf")
print(model)
...
library(rpart)
model_rpart <- rpart(Loan_Status ~ ., data = new_train, method = "class")
summary(model_rpart)
...
########################## predicting on svm
pred_rpart_test <- predict(tunedModel,as.matrix(final_test_sd))
final_result_svm <- data.frame(pred_rpart_test)
